#!/usr/bin/env python3
"""
《光遇》.mesh 批量转 OBJ (普通模型 + 压缩模型 + 自动后备)
功能：
  - 自动识别并解析普通 .mesh 文件（启发式偏移）
  - 根据 MeshDefs.lua 中的 compressPositions/compressUvs 标志，自动调用压缩模型解析器
  - 若启发式解析失败，自动尝试压缩解析器作为后备
  - 支持交互式多选（序号/范围/逗号/空格/all）和自定义输出目录
...
def parse_compressed_mesh(f, lz4):
    """
    解析启用了 compressPositions/compressUvs 的模型
    """
...
def main():
    ...
    # 特殊关键词列表（用于文件名启发）
    special_keywords = ['StripAnim', 'CompOcc', 'ZipPos', 'ZipUvs', 'StripNorm']
